...
3. Bind an `OtpChannelSender` to deliver codes (use `OtpMessageFormatter::format()` with the config `message_template` for byte-exact SMS copy). To reject the request when the gateway fails instead of issuing a challenge for an unsent code, implement `OtpDeliveryAwareSender` too and return `OtpDeliveryResult::failure(...)` — the broker deletes the challenge and responds 502.
